...
Jawab : file image yang dihasilkan biasanya bernama
xinu.bin atau terkadang hanya xinu, File tersebut berada di dalam folder `xinu/compile/`

### 2. Membaca source code Xinu
Jawab :![alt text](img/image-12.png)
...
Jawab :  Struktur data proses pada Xinu OS, yang dikenal sebagai Process Control Block (PCB), disimpan dalam file header xinu/include/process.h. Dalam file ini, struktur tersebut didefinisikan dengan nama struct procent (singkatan dari process entry), Informasi yang disimpan dalam struct procent mencakup berbagai atribut vital, mulai dari prstate yang mencatat status proses (seperti sedang berjalan, siap, atau sedang tidur), hingga prprio yang menentukan tingkat prioritas proses tersebut dalam antrean CPU.

### 4. Mengubah Welcome Banner pada Xinu 

a. **Mencari File Penyimpan Banner**  
   Carilah file yang menyimpan banner Xinu!  
   *Hint: File berekstensi `.h` pada direktori `xinu/include`.*

b. **Mencari File Penampil Banner**  
   Carilah file yang menampilkan banner Xinu!  
   *Hint: File berekstensi `.c` pada direktori `xinu/shell`.*

c. **Modifikasi Welcome Banner**  
   Ubahlah welcome banner Xinu sesuai dengan selera Anda.  

   **Instruksi:**  
   Tambahkan **NAMA** dan **NIM** masing-masing pada tampilan tersebut.

d. **Setelah melakukan perubahan, jalankan perintah berikut pada terminal Linux:**
